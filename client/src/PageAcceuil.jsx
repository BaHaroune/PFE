  ...
  useEffect(() => {
    Axios.get('http://localhost:3001/login').then((response) => {
      if (response.data.loggedIn && response.data.user) {
        setLoginStatus(response.data.user.nom);
  ...
  return (
    <Router className=" justify-center items-center my-10">
      <div className='bg-[#000300] w-full px-5'>
        <nav className='flex justify-between items-center w-full text-white p-5'>
          <Link to="/">
          ...
          <div>
            <div className='flex justify-between items-center'>
              {loginStatus !== ''
                ?
                <div className='bg-[#00df9a] font-bold mx-2 px-2 rounded-full'><p className='font-bold text-1xl'>{loginStatus}</p></div>
                :
                <div>
                  <Link className='m-3 font-bold hover:text-[#00df9a]' to='/login' >login</Link>
                  <Link className='m-3 font-bold hover:text-[#00df9a]' to='/signup' >Sign Up</Link>
